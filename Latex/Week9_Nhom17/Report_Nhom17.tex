...
\begin{document}
\title{\textbf{\MakeUppercase{Dự báo giá của tiền điện tử bằng phương pháp chuỗi thời gian}}}
...
\maketitle
\textbf{TÓM TẮT NỘI DUNG} \> Nghiên cứu này sử dụng một loạt các mô hình dự báo để phân tích và dự đoán tỷ giá trao đổi của Binance Coin (BNB), Bitcoin (BTC), và Ethereum (ETH) so với USD từ năm 2019 đến 2024. Bằng cách sử dụng các mô hình như TimesNet, Random Forest, CNN-LSTM, XGBoost, và VAR để dự báo các biến động, cùng với hồi quy tuyến tính, nghiên cứu của chúng tôi nhằm cung cấp những thông tin quý báu cho các nhà đầu tư, các nhà phân tích tài chính, và những nhà quyết định chính sách trong việc điều hướng thị trường tiền điện tử dao động mạnh mẽ.\\
\textbf{TỪ KHÓA} \> Chuỗi thời gian, phương pháp thống kê, tỷ giá trao đổi tiền điện tử, học máy, học sâu
\textbf{ \section{GIỚI THIỆU}}
...
TimeNets & 7:3 & 1339.91 & 2.28 & 1795377.01 \\
& 8:2 & 1666.05 & 2.3 & 2775746.13 \\
& 9:1 & 3084.43 & 3.71 & 9513722.12 \\ \hline
\end{tabular}
\end{adjustbox}12
\end{table}
...
TimeNets & 7:3 & 86.12 & 2.4 & 7417.96 \\
& 8:2 & 105.27 & 2.6 & 11083.64 \\
& 9:1 & 163.58 & 3.5 & 26761.49 \\ \hline
\end{tabular}
\end{adjustbox}
\end{table}\\
49NHẬN XÉT: Dựa vào các chỉ số đánh giá độ chính xác trên
từng mô hình và trên cả 3 bộ dữ liệu BNB-USD, BTC-USD,
ETH-USD, Những mô hình có dộ chính xác cao nhất là
XGBoost, Random Forest và Linear Regression. Các mô hình này hoạt
động tốt trên bộ dữ liệu TIỀN ĐIỆN TỬ, thích hợp để dự đoán 1 cách chính xác\\
\textbf{ \section{TỔNG KẾT}}\\
Trong nghiên cứu về dự báo tỷ giá tiền điện tử (BNB, BTC, ETH) so với USD, các mô hình thống kê, học máy và học sâu đã được áp dụng để cải thiện độ chính xác của dự báo. Nhóm nghiên cứu đã thực hiện và so sánh nhiều phương pháp khác nhau, bao gồm cả các phương pháp truyền thống như Linear Regression và các mô hình học máy như Random Forest, XGBoost, và VAR (Vector Autoregression), cũng như các mạng nơ-ron sâu như LSTM, GRU, TimesNet và CNN-LSTM.\\
Kết quả cho thấy rằng các mô hình như Linear Regression, XGBoost và Random Forest đã cho thấy độ chính xác vượt trội trong việc xử lý các dữ liệu thời gian và dự báo các biến động phức tạp của giá tiền điện tử so với USD. Điều này cho thấy tiềm năng lớn của các phương pháp học máy và thống kê trong dự báo thị trường tiền điện tử.\\
Tuy nhiên, cần lưu ý rằng sự lựa chọn mô hình phụ thuộc vào bối cảnh cụ thể và đặc tính của dữ liệu. Các mô hình học sâu như LSTM và GRU thường phù hợp khi dữ liệu có tính chuỗi thời gian phức tạp và các mối quan hệ phi tuyến tính, trong khi Linear Regression, XGBoost và Random Forest có thể cung cấp các dự báo tốt khi mối quan hệ giữa các biến đơn giản hơn và dữ liệu ít cấu trúc hơn.\\
\textbf{ \section{ĐỊNH HƯỚNG TƯƠNG lAI}}\\
Mở rộng và cải thiện bộ dữ liệu: Nâng cao chất lượng và độ phong phú của bộ dữ liệu bằng cách tích hợp nhiều nguồn dữ liệu khác nhau và cải thiện quy trình thu thập dữ liệu để giảm thiểu thiếu sót.\\
Nghiên cứu thêm về phương pháp tiền xử lý: Tăng cường nghiên cứu và áp dụng các phương pháp tiền xử lý dữ liệu tiên tiến để làm sạch và chuẩn bị dữ liệu tốt hơn cho quá trình huấn luyện mô hình.\\
Nghiên cứu và phát triển mô hình phù hợp hơn: Tiếp tục nghiên cứu và thử nghiệm các mô hình mới, bao gồm cả sự kết hợp giữa các phương pháp học máy và học sâu, để tăng cường khả năng dự báo và độ chính xác.\\
Tối ưu hóa và đánh giá mô hình: Tăng cường quá trình tối ưu hóa mô hình và sử dụng các phương pháp đánh giá hiệu suất tiên tiến để đảm bảo rằng mô hình dự báo đáp ứng được yêu cầu và đạt được kết quả tốt nhất có thể.\\
\section*{LỜI CẢM ƠN}\\
...
\bibitem{b13} Denny Britz, "Recurrent Neural Networks Tutorial, Part 1 – Introduction to RNNs", Denny's Blog, 2015-09-17, [https://dennybritz.com/posts/wildml/recurrent-neural-networks-tutorial-part-1/]
\bibitem{b14} Xingyu Yang, "A CNN-LSTM Model Based on a Meta-Learning Algorithm to Predict Groundwater Level in the Middle and Lower Reaches of the Heihe River, China", Water 2022, 14(15), 2377, 31-07-2022, [https://www.mdpi.com/2073-4441/14/15/2377]
\bibitem{b15} Jason Brownlee PhD, “Random Forest for Time Series Forecasting”, on November 1, 2020. [https://machinelearningmastery.com/random-forest-for-time-series-forecasting/]
\end{thebibliography}
