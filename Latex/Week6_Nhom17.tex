...
\begin{itemize}
	\item Cổng đặt lại: $r_t = \sigma(W_{xr} x_t + W_{hr} h_{t-1} + b_r)$.
	\item Cổng cập nhật: $z_t = \sigma(W_{xz} x_t + W_{hz} h_{t-1} + b_z)$.
	\item Trạng thái dự kiến: $\tilde{h}_t = \tanh(W_{xh} x_t + W_{hh} (r_t \odot h_{t-1}) + b_h)$.
	\item Trạng thái ẩn cuối cùng: $h_t = z_t \odot h_{t-1} + (1 - z_t) \odot \tilde{h}_t$.
\end{itemize}
